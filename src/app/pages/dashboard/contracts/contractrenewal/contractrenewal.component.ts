...
import { AddcontractService } from '../addcontract/services/addcontract.service';
...
ngOnInit(): void {
  ...
  this.editBehaviorService.idSubscribe.pipe(takeUntilDestroyed(this.destroyRef)).subscribe((id:any)=>{
    if(id){
        this.btnAddandUpdate = 'add';       // ✅ تحويل المود إلى تحديث
        this.idUpdate = id;
        
         this.contractsForm.get("ContractDate")?.clearValidators();
      this.contractsForm.get("ContractDate")?.updateValueAndValidity();
        // this.
...
this.showBtns = true;
  ...
  this.contractsForm.valueChanges.subscribe((res:any)=>{
    ...
    this.contractsForm.patchValue({ LeaseMonths: '' }, { emitEvent: false });
    this.monthsList = [];
    return;
}
    // if(res.LeaseStartDate && res.LeaseEndDate){
...
this.formDataTenant.patchValue({
  ...
  getMonthsDiff(startDate: Date, endDate: Date):number{
    let monthsCount1:any=(endDate.getFullYear()-startDate.getFullYear())*12;
    monthsCount1+=endDate.getMonth()-startDate.getMonth();
    return monthsCount1;
  }


  // getMonthsList(startDate: Date, endDate: Date):number[]{
  //   let monthsList:number[]=[];
  //   let startDateCopy = new Date(startDate);
  //   let endDateCopy = new Date(endDate);
  //   while(startDateCopy.getTime() < endDateCopy.getTime()){
  //     monthsList.push(startDateCopy.getMonth());
  ...
  // getNet():number{
  //   let contractValue = this.contractsForm.get("ContractValue")?.value;
  //   let taxAmount = this.contractsForm.get("TaxAmount")?.value;
  //   let brokerCommission = this.contractsForm.get("BrokerCommission")?.value;
  //   let otherCommission = this.contractsForm.get("OtherCommission")?.value;
  //   let insuranceValue = this.contractsForm.get("InsuranceValue")?.value;
  //   let netAmount = contractValue - taxAmount - brokerCommission - otherCommission - insuranceValue;
  //   this.contractsForm.get("NetAmount")?.setValue(netAmount);
  //   return netAmount;
  // }

...
export class ActivitytrackingComponent {
  ...
  // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! Services
  activityTrackingService:ActivitytrackingService=inject(ActivitytrackingService)
...
searchUserId(e:any){
 ...
 if (/[a-zA-Z\u0600-\u06FF]/.test(e.value)) {
    ...
    if(!/[a-zA-Z\u0600-\u06FF]/.test(e.value)){
      this.toastr.show('اسم المستخدم يجب أن يحتوي على حروف فقط او حروف وارقام', 'error');
      e.value = e.value.replace(/[a-zA-Z\u0600-\u06FF]/g, "");
      return;
    }
  }
  this.activityTrackingService.searchUser(valueSearch).pipe(takeUntilDestroyed(this.destroyRef)).subscribe((res:any)=>{
    // this.usersData=res
...
onSubmit(){
  ...
  endDate: this.formData.get('endDate')?.value
    ...
    this.activityTrackingService.logs(data).pipe(takeUntilDestroyed(this.destroyRef)).subscribe((res:any)=>{
      // this.logsData=res

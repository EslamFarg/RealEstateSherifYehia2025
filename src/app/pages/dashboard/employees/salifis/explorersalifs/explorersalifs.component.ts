...
getAllDataPaymentVoucher(){
  ...
  this._salifsServices.getAllData(pagination).pipe(takeUntilDestroyed(this.destroyRef)).subscribe((res:any)=>{
    this.salifsData=res
...
switch(e.index){
  case 0 : ShapeDataFilter.searchRequest.searchFilter.column=0; break;
  case 1 : ShapeDataFilter.searchRequest.searchFilter.column=15; break;
}

  this._salifsServices.searchData(ShapeDataFilter).pipe(takeUntilDestroyed(this.destroyRef)).subscribe((res:any)=>{
    this.salifsData=res;
    console.log(res);
  })
...
deleteConfirmed(id:any){
  ...
  this._salifsServices.deleteData(id).pipe(takeUntilDestroyed(this.destroyRef)).subscribe((res:any)=>{

...
export class AddpaymentvoucherComponent {
  ...
  _ownerPaymentVoucherServices: OwnerpaymentvoucherService = inject(
    OwnerpaymentvoucherService
  );
  destroyRef: DestroyRef = inject(DestroyRef);
  toastr: ToastrService = inject(ToastrService);
  editBehaviorSubject: EditBehaviorServiceService = inject(
  ...
  @ViewChildren('remainingAmount')
  remainingAmountElement?: QueryList<ElementRef>;
  ...
  ngOnInit() {
    this.editBehaviorSubject.idSubscribe
      ...
      .subscribe((res: any) => {
        ...
        this._ownerPaymentVoucherServices
          ...
          .subscribe((res: any) => {
            ...
            this.TotalpaidTaxAmount =
              this.totalAmount / (1 + this.getTax / 100);
  ...
  onPaidAmountChange(event: any, item: any) {
    ...
    if (value > 0) {
      ...
      const checkbox = this.oneCheckElement?.find(
        (el) => Number(el.nativeElement.dataset.id) === id
      );
  ...
  checkDataItem(e: any, item: any) {
    ...
    if (checked) {
      const index = this.itemChecked.findIndex(
        (x) =>
          x.contractInstallmentId === item.contractInstallmentId ||
          x.contractInstallmentId === item.id
      );
      if (index === -1) {
        this.itemChecked.push({
          contractInstallmentId: item.contractInstallmentId ?? item.id,
      ...
      this.itemChecked = this.itemChecked.filter(
        (x) =>
          x.contractInstallmentId !== (item.contractInstallmentId ?? item.id)
      );
    }
  ...
  onSubmit() {
    ...
    if (this.paymentVoucherForm.valid) {
      ...
      if (
        this.formDataSearch.get('financiallyAccountId')?.value == null ||
        this.formDataSearch.get('financiallyAccountId')?.value == undefined
      ) {
      ...
      let data = {
        ...
        ownerPaymentDetails: [] as {
          contractInstallmentId: number;
          amount: number;
        }[],
      };
      ...
      if (this.btnAddandUpdate == 'add') {
        this._ownerPaymentVoucherServices
          .createOwnerPaymentVoucher(data)
          .pipe(takeUntilDestroyed(this.destroyRef))
          .subscribe((res: any) => {
        ...
        this._ownerPaymentVoucherServices
          .updateOwnerPaymentVoucher(updateData)
          .pipe(takeUntilDestroyed(this.destroyRef))
          .subscribe((res: any) => {
...
getDataById(e:any){
  ...
  sendDataSelectedSearch(e: any) {
    ...
    this._ownerPaymentVoucherServices
      .getDatailsContracts(this.idSearchOwner, pagination)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe((res: any) => {
        this.paymentData = {
